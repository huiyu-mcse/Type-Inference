...
class State {
  ...
  regConstraint(lhs, rhs) { this._reg(lhs); this._reg(rhs); }
  ...
  _setBase(rx, type, ctx) {
    ...
    if (cur && cur !== type)  this.errors.push(`CONFLICT: ${cur} vs ${type}  (${ctx})`);
    else if (!cur) {
      if (this.isObj.get(rx)) this.errors.push(`CONFLICT: obj vs ${type}  (${ctx})`);
      else                    this.baseType.set(rx, type);
    }
  }

  _initObj(rx) {
    if (this.baseType.has(rx)) { this.errors.push(`CONFLICT: ${this.baseType.get(rx)} vs obj`); return; }
  ...
  displayState() {
    ...
    for (const node of this.displayList) {
      ...
      if (isLiteral(node)) { parts.push(`${node}:bot`); continue; }
